...
while True:
    ...
    for i in range(loopsize):
        ...
        if (str(classes[redundant[i]])=='person'):
            
            count = str((countlists[i]))
            cv2.putText(image_new , 'Worker Count:', (x, y), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (255, 255, 255), 1)
            x += 150
            cv2.putText(image_new , count, (x, y), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 255), 1)
            x = x - 150
            y += 40
        elif(str(classes[redundant[i]])=='car') :
            count = str((countlists[i]))
            cv2.putText(image_new , 'vehicles Count:', (x, y), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (255, 255, 255), 1)
            x += 155
            cv2.putText(image_new , count, (x, y), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 255), 1)
            x = x - 155
            y += 40
    ...
    if(loopsize==0):
        cv2.putText(image_new , 'Worker Count: 0', (x, y), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (255, 255, 255), 1)
        y += 40
        cv2.putText(image_new , 'Vehicles Count:0', (x, y), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (255, 255, 255), 1)
    # elif (str(classes[redundant[i]])!='person'):
